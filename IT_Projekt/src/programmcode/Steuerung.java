...
public class Steuerung implements Runnable{
  ...
  private SchreibenLesen sl;
  private boolean speichern = false;
  private int spielstand;
  private JButton btnSpielstandSpeichern;
  private JButton btnSpielstandLaden;
  private boolean spielstandSpeichern;
  
  public Steuerung() {	  
	  keyevt = new KeyEvt(this);
	  screen = new Screen(this, keyevt);
	  sl = new SchreibenLesen(this);
	  btnSpielstandSpeichern = screen.getBtnSpielstandSpeichern();
	  btnSpielstandLaden = screen.getBtnSpielstandLaden();
  }
 ...
 public void neueRunde() {
  ...
  public void spielstandXSpeichern() {
	  sl.schreiben(spielfeld, kreise, spielstand);
  }
  
  public void setKreise(int pKreise) {
	  kreise = pKreise;
  }
  
  public void spielstandLaden() {
	  try {
		spielfeld = sl.lesen(spielstand);
	} catch (IOException e) {
  ...
  public void spielstandSpeichern(int pY) {
	  spielstand = (pY-180)/100;
	  System.out.println(spielstand);
	  btnSpielstandSpeichern.setText("Spielstand " + spielstand + " speichern");
	  btnSpielstandLaden.setText("Spielstand " + spielstand + " laden");
	  spielstandSpeichern = true;
  }
  ...
  public void render() {
    ...
    JButton[] btnSpielstaende = screen.getBtnSpielstaende();
    ...
    for(int i = 0; i<kreis.size();i++) {
    	...
    	for(int i = 0; i<5; i++) {
    		...
    		if(!spielstandSpeichern) {
    		for(int i = 0; i<5; i++) {
    			btnSpielstaende[i].setVisible(true);
    			btnSpielstaende[i].setEnabled(true);
    		}
    		btnEnde.setEnabled(false);
        	btnEnde.setVisible(false);
        	btnNeuesSpiel.setEnabled(false);
    		...
    		btnSpielstandSpeichern.setEnabled(true);
    		btnSpielstandSpeichern.setVisible(true);
    		btnSpielstandLaden.setEnabled(true);
    		btnSpielstandLaden.setVisible(true);
    	}
    	}
    }
